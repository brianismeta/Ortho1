...
function GetOrthoJSON(index) {
  ...
  ethereum
    ...
    .then((result) => {
      ...
      orthoverseJSON = decodeURIComponent(
...
function GetOrthoCastleLevel(index) {
  ...
  ethereum
    ...
    .then((result) => {
      ...
      // Using the decodeURIComponent function in combination with regex to convert the hex value into utf8 value
      orthoverseCastleLevel = convertHexToDecimal(result.substr(2));
      // decodeURIComponent(
      //   result.substr(130).replace(/\s+/g, "").replace(/[0-9a-f]{2}/g, "%$&")
      // ).replace(/\0/g, '');//.trim();
        //var xy = symbol.replace(/\0/g, '') ;
...
function RefreshButtonStatus() {
  ...
  if (connectStatus == _connectStatus.Connected) {
    ...
    if(walletID) walletID.innerHTML = `Need to switch your network to Ethereum MainNet`;
    if(walletID) walletID.classList.remove("d-none");//.style.display="";
...
//    RefreshButtonStatus();
  ...
  if (playButton) playButton.addEventListener("click",() => {
    location.href = "metameowOrthoLobby.html";
  });
    if (orthoButton) orthoButton.addEventListener("click", ()=> {
//    AddOrthoTokenToWallet();
    var json = GetOrthoJSON(0);
    var level= GetOrthoCastleLevel(0);
